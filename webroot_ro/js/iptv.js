...
function addList() {
	...
	if (!(/^[0-9]{1,}$/).test(vlan)) {
		...
		return ;
	} else if (parseInt(vlan, 10) > 4094 || parseInt(vlan, 10) < 4 || (parseInt(vlan, 10) >= 4000 && parseInt(vlan, 10) <= 4016)) {
		showErrMsg("msg-err",  _("The VLAN ID range is %s.", ["4-3999, 4017-4094"]));
		return;
...
function initValue(obj) {
	...
	G.listMax = len;
	top.initIframeHeight();
	if (top.G.countryCode !== "CN") {
		$("#iptvType").find("option[value=shanghai]").hide();
	}
}
