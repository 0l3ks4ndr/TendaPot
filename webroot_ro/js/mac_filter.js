...
function addTableList(tableId) {
	...
	if(mac == G.localhostMac){
		showErrMsg("msg-err", _("The MAC address of the local device cannot be blacklisted."));
		return;
	}
	//删除加入白名单操作
...
function initValue(obj) {
	...
	$.getJSON("goform/getOnlineList?" + Math.random(), function(data){
		G.localhostMac = data[0].localhostMac
	});
