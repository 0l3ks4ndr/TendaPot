...
var view = R.moduleView({
	initEvent: function () {
		$("#adv_mode, #adv_mode_5g").on("change", function () {
			getBandWidthList();
			specialLogic();
		});
		...
		$("#adv_band").on("change", specialLogic);

		$("#adv_channel").on("change", function() {
        ...
        try {
            var _app = JSON.parse(window.sessionStorage.getItem('_app_info'));

            if (_app.workMode === 'wisp' || (_app.workMode !== 'ap' && _app.workMode !== 'router')) {
                $('#adv_channel').attr('disabled', true);
                $('#adv_channel_5g').attr('disabled', true)
                $('#adv_band').attr('disabled', true);
                $('#adv_band_5g').attr('disabled', true)
            }
...
var moduleModel = R.moduleModel({
	...
	getSubmitData: function () {
        if ($('#adv_channel').attr('disabled')) {
            toggleFormDisabled(false);

            var result = $("#wireless").serialize();

            toggleFormDisabled(true);

            return result;
        } else {
            return result = $("#wireless").serialize();
        }
	}
});
...
function toggleFormDisabled(value) {
    $('#adv_channel').attr('disabled', !!value);
    $('#adv_channel_5g').attr('disabled', !!value)
    $('#adv_band').attr('disabled', !!value);
    $('#adv_band_5g').attr('disabled', !!value);
}
...
/**
 * 业务逻辑处理，仅相关2.4g
 * 如果频宽选项为 40, 20/40, auto，则选项隐藏14信道，且当当前信道为14时重置为自动信道0
 */
function specialLogic() {
	setTimeout(function() {
		if (($("#adv_band").val() || '').indexOf('40') === -1 && $("#adv_band").val() !== "auto") {
			$("#adv_channel option[value=14]").show();
			return;
		}

		$("#adv_channel option[value=14]").hide();

		if ($("#adv_channel").val() === '14') {
			$("#adv_channel").val(0); // 重置为自动
		}
	}, 0);
}
