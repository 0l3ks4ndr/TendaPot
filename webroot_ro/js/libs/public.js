...
(function ($) {
	...
	$.ajax = function (option) {
		...
		if (top.GOAHEAD_AES_CRYPT == "y") {
			var noEncryptReqApi = ["Auth"] ,//不加密的post接口
			reqEncrypt= true;
			noEncryptReqApi.forEach((item)=>{
				if(option.url.indexOf(item)!=-1){
					reqEncrypt = false
				}
			})
			if(option.type.toLowerCase() == "post" && reqEncrypt){
				if (typeof option.data === "object") {
					option.data = JSON.stringify(option.data);
				} 
				option.data = top.Encrypt(option.data);
			}
			//使用$.getJSON发送的请求，返回密文后 不会进入option.success，所以将JSON请求的accept类型修改为普通请求
		...
		//调试用 获取假数据
		// option.url = option.url.split("?")[0]+".txt?"+option.url.split("?")[1]
		...
		option.success = function (res) {
			...
			if (top.GOAHEAD_AES_CRYPT == "y" && res.toString().indexOf("<!DOCTYPE") == -1) {
				if (ifDecrypt(option)) {
					res = top.Decrypt(res);
				}
				//将原为json请求的返回数据 解析成json格式后再返回
				if (((option.dataType||"").toLowerCase() == "json") || option.isJson) {
					res = JSON.parse(res)
				}
			}
...
function ifDecrypt(option){
	//不用解密的接口
	var noEncryptResApi = ["getLoginInfo","getRebootStatus","stokCfg"], resEncrypt = true;
	noEncryptResApi.forEach((item)=>{
		if(option.url.indexOf(item)!=-1){
			resEncrypt = false;
		}
	})
	return resEncrypt && option.url.indexOf("goform")>-1;
}
