    ...
    valid = {
        ...
        hex: function (str) {
            if (!(/^[0-9a-fA-F]{1,}$/).test(str)) {
                return _("Must be hex characters");
            }
        },
        ...
        ipv6prefix: {
            all: function (str) {
                var items = str.split("::"),
                    ret = $.validate.valid.ipv6Header.all(str);
                if(ret){
                    return ret;
                }
        ...
        ipv6Header: {
            ...
            specific: function (str) {
                var headStr = str.split("::")[0].split(":")[0],
                    ...
                    errAddrHeader = ["FE80", "FEC0", "FC00", "FD00", "2002"],
                    i = 0;
                if (headStr.length != 4) {
                    return (_("Please enter a valid address"));
                }else {
                ...
                for(i; i < errAddrHeader.length; i++){ //全球唯一的IPV6地址非法开头："FE80", "FEC0", "FC00", "FD00", "2002"
        ...
        addValidateTip: function (str) {
            ...
            function createTipElem(id, str, elem) {
                var tipElem = document.createElement('span'),
                    ...
                    span;
                tipElem.className = 'validatebox-outer-span'
            ...
            return this.each(function () {
                ...
                var inputWidth = Math.max(this.getBoundingClientRect().width, 240);
                $tipElem = $(tipElem).css({
                    "position": "absolute",
                    "width": inputWidth + 'px',
                    'top': '30px',
                ...
                //动态计算设置-marginBottom
                var height = 20;
                var dom = $(this).parent().parent().find('.validatebox-outer-span');
                for (var i = 0; i < dom.length; i++) {
                    height = Math.max(dom[i].getBoundingClientRect().height, height)
                }
