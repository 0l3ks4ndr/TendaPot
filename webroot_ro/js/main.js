...
function PageLogic(pageView, pageModel) {
    ...
    this.changeMenu = function (curId, targetId) {
        ...
        if(!window.CONFIG_VPN || (window.CONFIG_VPN && window.CONFIG_VPN == "n")) {
            menus.remove("vpn-setting");
        }
    ...
    this.onMenuClick = function (e) {
        ...
        if("nav-logo" == e.target.className) {
            ...
            if(B.getLang() == "cn") {
                tendaOfficail = "http://www.tenda.com.cn";
            }
        ...
        // 通过计算统一兼容，避免加一个语言又需要重新查看的问题
        var boxLen = $('.main-area').length;
        for (var i = 0; i < boxLen; i++) {
            var boxWidth = $('.main-area')[i].getBoundingClientRect().width;
            var text = $('.main-area .function-title')[i].innerText;
            var titleWidth = this.getTextWidth(text);
            var id = "#" + $('.main-area')[i].id
            if (titleWidth > boxWidth) {
                $(id + " .function-status").css("visibility",'hidden');
                $(id + " .function-title").addClass("white-space");
            }else{
                $(id + " .function-status").css("visibility", 'visible');
                $(id + " .function-title").removeClass("white-space");
            }
        }
    };

    this.getTextWidth = function(str) {
        $('body').append($('<span class="clac-text-width function-title" style="visibility: hidden; position:absolute;">' + str + '</span>'))
        var width = $('.clac-text-width')[0].getBoundingClientRect().width
        $('.clac-text-width').remove();
        return width;
    }
...
staInfo = {
    ...
    setImage: function (obj) {
        ...
        window.sessionStorage.setItem('_app_info', JSON.stringify(_app_info));
...
netInfo = {
    ...
    init: function () {
        ...
        if (!netInfo.loading) {
            ...
            if(window.CONFIG_SELECT_ISP && window.CONFIG_SELECT_ISP == "y") {
        ...
        // 是否需要ISP切换 未定义或值为n时都隐藏
        if(!window.CONFIG_SELECT_ISP || (window.CONFIG_SELECT_ISP && window.CONFIG_SELECT_ISP == "n")) {
            $("#isp_type").hide();
        }
        netInfo.initHtml()

        $("#gateway, #gateway2").attr("data-options", '{"type":"ip","msg":"' + _("Please enter a correct gateway IP address.") + '"}');
        $("#dns1, #dns1").attr("data-options", '{"type":"ip","msg":"' + _("Please enter a correct primary DNS server.") + '"}');
        $("#dns2, #dns2").attr("data-options", '{"type":"ip","msg":"' + _("Please enter a correct secondary DNS server.") + '"}');
    ...
    preSubmit: function () {
        ...
        if(window.CONFIG_SELECT_ISP == "y") {
            if( (wanData.ispType != ispType) ||wanData.wanType != wanType || ((ispType == "2" || ispType == "6" || ispType == "7") && wanData.ispAreaz != ispArea) || (ispType == "3" && (wanData.wanVlanId != wanIdVal || wanData.lanVlanId != lanIdVal)) || netInfo.initObj.multiWanEn != ($("#multiWanEn").hasClass("btn-on") ? "true" : "false")){
    ...
    initHtml: function () {
        if (window.CONFIG_SELECT_MALY === 'y') {
            $("#internet-setting #internet-form #wan1SetWrap #isp_set #ispType").append($(
                '<option value="1">Unifi</option>\n' +
                '<option value="2">Maxis</option>\n' +
                '<option value="6">Celcom</option>\n' +
    ...
    init: function () {
        if (!parentInfo.loading) {
            ...
            $("#parental_list").on("click", ".enable", function () {
                ...
                $.post("goform/parentControlEn", objTostring(params));
            });
...
ipv6Info = {
    ...
    init: function () {
        ...
        if (!ipv6Info.loading) {
			...
			$.validate.valid.wanAddr = {
            all: function (str) {
                if ((str.indexOf("2001:3") !== -1) || (str.indexOf("2001:03") !== -1) || (str.indexOf("2001:003") !== -1) || (str.indexOf("2001:0003") !== -1)) {
                    // return 'error! wan and lan in the same network segment '
                    var wanStr = _("WAN IP Address");
                    var lanStr = _("LAN IP Address");
                    return _("%s and %s (%s) must not be in the same network segment.", [lanStr, wanStr, str]);
                } else {
                    return $.validate.valid.ipv6.all(str);
                }
            }
        	}
        }
...
var goExit = '<span class="go-exit" style="float: left"><a style=" color: #999;" href="goform/exit">'+_('Exit')+'</a></span>';
