...
function backSetWifi () {
    ...
    $("#wifi_setting").removeClass("none");
    $("#btn_control, #step-next").removeClass("none");
    $("#wrlPassword").focus();
    $.setCursorPos(document.getElementById("wrlPassword"), document.getElementById("wrlPassword").value.length);
}
...
function showFinish () {
    ...
    if (G.wanStatus == "7") {
        ...
        if (G.wanStatus == 7) {
            ...
            // 间隔3秒后未联网时，跳转到主页  添加条件，不用重启时才需要3s后跳转
            if (G.accessType == 1 && $("#ispType").val() === "0") {
                setTimeout(function () {
...
function overStep () {
    clearTimeout(G.time);
    $("#step-next").removeClass("none").val(_("Next"));
    if (!$("#net_setting").hasClass("none")) {
		G.skipInsertLine = true;
		$("#net_setting").addClass("none");
		$("#internet").removeClass("none");
		$("#netWanType").val("2");
		wanTypeSelect("2");
